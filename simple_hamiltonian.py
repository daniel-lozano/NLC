...
q=2*np.pi*np.arange(-2.501, 2.501, 0.025)
...
def gen_hamiltonian(basis,N,Jzz,Jpm,h,NN):
...
print "shape of symmetry",SYM.shape[0]
for c in range(len(cluster)-5):
    print c,"Initialicing calculus for cluster type ", cluster[c]
    ...
    H=gen_hamiltonian(basis,N,Jzz,Jpm,h,NN)
    ...
    #At this point the prefactor due to the direction and the exponential factor must be multiply by the average factor previously found to have the total scattering function, the different type of cluster must be included aswell
        ...
        for sym in range(SYM.shape[0]):
            ...
            Projection_factor=np.dot(z_1,Z_scatt)*np.dot(z_2,Z_scatt)

            for h,l in it.product(range(q.size),range(q.size)):
                
                q_vector=np.array([q[h],q[h],q[l]])/np.sqrt(2*q[h]**2+q[l]**2)

                c_NSF_intensity[c][l,h]+=Op_T_average*np.cos(q_vector*r_ij)*Projection_factor/48.
